...
public class UpdateEmployeeBusinessTest extends BaseTest {
    ...
    public void updateAllFieldsTest() throws Exception {
        step("Обновление сотрудника по всем полям ", step -> {
        ...
        step("Проверка обновленного города", step -> {
            assertEquals(CITY_2, updatedEmployee.getCity());
        });
        step("Проверка обновленного имени", step -> {
            assertEquals(FIRSTNAME_2, updatedEmployee.getName());
        });
        step("Проверка обновленной должности", step -> {
            assertEquals(POSITION_2, updatedEmployee.getPosition());
        });
        step("Проверка обновленной фамилии", step -> {
    ...
    public void updateSurnameCityFieldsTest() throws Exception {
        step("Обновление сотрудника по фамилии и городу ", step -> {
        ...
        step("Проверка, что имя у сотрудника не поменялось", step -> {
            assertEquals(FIRSTNAME, updatedEmployee.getName());
        });
        step("Проверка, что должность у сотрудника не поменялась", step -> {
    ...
    public void updateNonExistentEmployeeTest() throws Exception {
        step("Обновление несуществующего сотрудника по всем полям", step -> {
    ...
    public void updateWithEmptyFieldsTest() throws Exception {
        ...
        assertTrue(result);
        step("Проверка, что город поменялся на пустое значение", step -> {
            assertEquals("", employeeAfterUpdate.getCity());
        });
        step("Проверка, что имя поменялось на пустое значение", step -> {
            assertEquals("", employeeAfterUpdate.getName());
        });
        step("Проверка, что должность поменялась на пустое значение", step -> {
            assertEquals("", employeeAfterUpdate.getPosition());
        });
        step("Проверка, что фамилия поменялась на пустое значение", step -> {
    ...
    public void updateWithSameDataTest() throws Exception {
        ...
        assertTrue(result);
        step("Проверка, что город не поменялся", step -> {
            assertEquals(CITY, employeeAfterUpdate.getCity());
        });
        step("Проверка, что имя не поменялось", step -> {
            assertEquals(FIRSTNAME, employeeAfterUpdate.getName());
        });
        step("Проверка, что должность не поменялась", step -> {
            assertEquals(POSITION, employeeAfterUpdate.getPosition());
        });
        step("Проверка, что фамилия не поменялась", step -> {
